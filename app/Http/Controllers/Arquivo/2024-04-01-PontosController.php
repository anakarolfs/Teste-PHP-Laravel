...
*/
    ...
    public function buscarResumoGeralPontos()
    {

        $resumo = DB::select('SELECT * FROM programa_pontos.fn_resumo_extrato_moedas() ORDER BY total_moedas DESC');

        foreach ($resumo as $operador) {
            $nome = DB::table('portal.vw_empregados')
                ->select('nome')
                ->where('login_dac', $operador->mat_usuario)
                ->first();

            // $operador->nome = $nome->nome . '' == '' ? '-' : $nome->nome . '';
            // $operador->mat_usuario = $operador->mat_usuario . '' == '' ? '-' : $operador->mat_usuario . '';
            // $operador->total_moedas = $operador->total_moedas . '' == '' ? '-' : $operador->total_moedas . '';
            // $operador->nivel_experiencia = $operador->nivel_experiencia . '' == '' ? '-' : $operador->nivel_experiencia . '';
            // $operador->de_nivel_experiencia = $operador->de_nivel_experiencia . '' == '' ? '-' : $operador->de_nivel_experiencia . '';
            // $operador->moedas_nivel = $operador->moedas_nivel . '' == '' ? '-' : $operador->moedas_nivel . '';
            // $operador->elogios = $operador->elogios . '' == '' ? '-' : $operador->elogios . '';
            // $operador->hora_extra = $operador->hora_extra . '' == '' ? '-' : $operador->hora_extra . '';
            // $operador->tma = $operador->tma . '' == '' ? '-' : $operador->tma . '';
            // $operador->outras_bonificacoes = $operador->outras_bonificacoes . '' == '' ? '-' : $operador->outras_bonificacoes . '';
            // $operador->compras = $operador->compras . '' == '' ? '-' : $operador->compras . '';
            // $operador->glosa_de_qualidade = $operador->glosa_de_qualidade . '' == '' ? '-' : $operador->glosa_de_qualidade . '';
            // $operador->falta_postural = $operador->falta_postural . '' == '' ? '-' : $operador->falta_postural . '';
            // $operador->falta_justificada = $operador->falta_justificada . '' == '' ? '-' : $operador->falta_justificada . '';
            // $operador->falta_injustificada = $operador->falta_injustificada . '' == '' ? '-' : $operador->falta_injustificada . '';
            // $operador->outras_deducoes = $operador->outras_deducoes . '' == '' ? '-' : $operador->outras_deducoes . '';
        }
    ...
    public function buscarVencedoresLeilao()
    {

        $vencedores = DB::table('programa_pontos.vw_vencedor_leilao as v')
            ->select(
                'p.no_produto as produto',
                'p.de_produto as complemento',
                ...
                'v.id_produto',
                'v.vr_lance',
                'v.mat_lance as matricula',
                'v.dt_lance',
                'v.st_lance',
                'v.fim_leilao'
            )
            ->leftJoin('programa_pontos.vw_produtos as p', 'v.id_produto', '=', 'p.id_produto')
            ->leftJoin('portal.vw_empregados as e', 'v.mat_lance', '=', DB::raw('e.login_dac::text'))
            ->orderBy('v.fim_leilao')
            ->get()
            ->toArray();
        ...
        foreach ($vencedores as $vencedor) {
            ...
            $vencedor->numero = $numero;

            $vencedor->dt_lance = new DateTime();
            $vencedor->dt_lance = $vencedor->dt_lance->format('d/m/Y (H:i:s)');

            $vencedor->fim_leilao = new DateTime();
            $vencedor->fim_leilao = $vencedor->fim_leilao->format('d/m/Y (H:i:s)');
        }
