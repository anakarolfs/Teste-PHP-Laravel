...
class ProdutosController extends Controller
{
    ...
    public function homeProduto()
    {
        ...
        $listProdutos1 = TbProdutosModel::select(
            'id_produto',
            'no_produto',
            'de_produto',
            'img_produto',
        ...
        $listProdutos = DB::table('programa_pontos.vw_produtos as P')
            ->select(
                'id_leilao', 'P.id_produto', 'no_produto', 'de_produto',  'img_produto', 'qt_estoque',
                'vr_produto', 'vr_lance', 'mat_lance', 'total_lance', 'st_produto', 'ic_status', 'diafechamento'
                //, 'st_desejos'
                , DB::raw("coalesce(ld.st_desejos, 0) as st_desejos")
                , 'dh_abertura', 'dh_fechamento', 'hh_abertura', 'hh_fechamento',
                'dt_abertura', 'dt_fechamento', 'nome'
            )
            ->leftJoin("portal.vw_empregados", "login_dac", "=",  DB::raw("CAST(mat_lance AS INTEGER)"))
            ->leftJoin("programa_pontos.tb_lista_desejos as ld", function ($join) {
                $join->on("ld.id_produto", "=", "P.id_produto");
                $join->on("ld.mat_inclusao", "=", DB::raw("'" . $_COOKIE['MATRICULA'] . "'"));
            })
            //->leftJoin("portal.vw_empregados", "login_dac", "=",  DB::raw("CAST(mat_inclusao AS INTEGER)"))
            ->orderByRaw('
                CASE
                    WHEN ld.st_desejos = 1 THEN 1
                    WHEN ic_status = 2 THEN 2
                    WHEN ic_status = 1 THEN 3
                    WHEN ic_status = 3 THEN 4
                    WHEN ic_status = 4 THEN 5
                    WHEN ic_status = 5 THEN 6
                    WHEN ic_status = 0 THEN 7
                END
            ')
            ->orderBy('st_produto', 'desc')->orderBy('qt_estoque', 'desc')->orderBy('vr_produto', 'asc')
            //->distinct()
            ->get(); //->paginate(10);
        //->toSql();
        // dd($listProdutos);
